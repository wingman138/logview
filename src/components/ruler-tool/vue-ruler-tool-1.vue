...
export default {
  ...
  props: {
    ...
    //true=mounted 时开自动启mousemove,mouseup,keyup,window.resize事件钩子
    //false=mounted 时不会自动启mousemove,mouseup,keyup,window.resize事件钩子，需要手动调用addEventHooks方法
    isScaleRevise: {
      type: Boolean,
      default: false
  ...
  mounted() {
    ...
    //   self.init()
    // }
    // 初始化 dom 尚未准备完成，获得到的尺寸可能不是实际尺寸，用nextticket也不行
    setTimeout(() => {
      this.init()
    }, 1000)
    // this.$nextTick(() => {
  ...
  methods: {
    ...
    box() {
      ...
      this.topSpacing = document.getElementById('levelRuler').getBoundingClientRect().top //.offsetParent.offsetTop
      this.leftSpacing = document.getElementById('verticalRuler').getBoundingClientRect().left // .offsetParent.offsetLeft
      // console.log(this.topSpacing, this.leftSpacing, this.windowWidth, this.windowHeight)
    }, // 获取窗口宽与高
    scaleCalc() {
    ...
    addEventHooks() {
      ...
      window.addEventListener('resize', this.windwoResize, true)
    },
    cleanEventHooks() {
      // console.log('cleanEventHooks')
      ...
      window.removeEventListener('resize', this.windwoResize, true)
    },
